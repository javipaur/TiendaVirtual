	...
	if ( $comments ) {  
		...
		foreach ($comments as $comment) {			
			...
			if ( current_user_can( 'edit_post', $comment->comment_post_ID ) ) 
				$deleteurl= '<a href="javascript:verify(\''.clean_url( wp_nonce_url(get_bloginfo('wpurl')."/wp-admin/comment.php?action=deletecomment".( $options['delete_spam'] ? "&dt=spam" : "")."&p=$comment->comment_post_ID&c=$comment->comment_ID", "delete-comment_$comment->comment_ID" ) ).'\', \'\')">'.htmlspecialchars  ($comment->comment_author).'</a>'; 
			else
				$deleteurl=htmlspecialchars  ($comment->comment_author);
		
			$author_data=get_userdatabylogin($comment->comment_author);
...
function WPWall_AdminMenu() {
	add_options_page('WP Wall Options', 'WP Wall ', 8, basename(__FILE__),'WPWall_Options');	
} 
...
<div class="wrap" style="">
	...
	<div id="sideblock" style="float:right;width:270px;margin-left:10px;"> 
		 <div class="ad">
					<a href="https://managewp.com/?utm_source=Plugins&amp;utm_medium=Banner&amp;utm_content=mwp250_2&amp;utm_campaign=WPWall" title="ManageWP.com - Manage your sites from one dashboard"><img src="$imgpath/mwp250_2.png" alt="ManageWP.com - Manage Multiple WordPress Sites"></a>
		 </div>
 	</div>
